...
async function checkPermissionStatus(): Promise<string> {
  ...
  try {
    ...
    console.error("Error checking permission status:", error instanceof Error ? error.message : "Unknown error")
    return "unknown"
  }
}

export async function getUserLocation(highAccuracyForced?: boolean): Promise<LocationData> {
  // Determinar si debemos usar alta precisión basado en el tipo de dispositivo
  // o si se ha forzado explícitamente
  const isMobile = isMobileDevice()
  const useHighAccuracy = highAccuracyForced !== undefined ? highAccuracyForced : isMobile
  ...
  if (typeof window !== "undefined" && "geolocation" in navigator) {
    ...
    if (permissionStatus === "denied") {
      console.log("Permiso de geolocalización denegado permanentemente, usando IP")
...
async function reverseGeocode(latitude: number, longitude: number): Promise<LocationData> {
  try {
...
async function getLocationByIP(): Promise<LocationData> {
  try {
    ...
    console.error("Error getting location by IP with ipapi:", error instanceof Error ? error.message : "Unknown error")
    ...
    try {
      ...
      const [latitude, longitude] = (fallbackData.loc || "0,0").split(",").map(Number)
...
export function requestLocationWithContext(
  message = "Para mostrarte contenido cercano, necesitamos acceder a tu ubicación.",
